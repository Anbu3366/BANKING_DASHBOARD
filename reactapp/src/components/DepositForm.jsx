...
const DepositForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setApiError(err.response?.data?.message || "Failed to process deposit. Verify if the server is reachable.")
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center space-x-2 text-sm text-muted-foreground bg-card/20 py-2 px-4 rounded-xl border border-border/40 w-fit">
        ...
        <span className="text-muted-foreground/50">/</span>
        <span className="text-foreground font-semibold">Deposit</span>
      </div>
      ...
      <div className="banking-card relative overflow-hidden border border-border shadow-2xl rounded-2xl p-8 bg-card/65 backdrop-blur-md">
        ...
        <div className="relative">
          <h2 className="text-3xl font-extrabold text-foreground mb-2 tracking-tight">Deposit Funds</h2>
          <p className="text-sm text-muted-foreground mb-6">Add money into your SecureBank account instantly</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="Enter deposit description (e.g. Salary, Gift, Cash)"
